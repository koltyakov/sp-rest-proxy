import { trimMultiline, waitForCondition } from './misc';
...
export const loadPageContext = async (proxyEndpoint?: string, timeout = 100, tries = 50): Promise<Partial<IPageContextInfo | undefined>> => {
  ...
  if (isLocalhost) {
    ...
    return Promise.all([ getWebInfo(), getSiteInfo() ]).then(([{ d: webInfo }, { d: siteInfo }]) => {
  ...
  SP.SOD.executeFunc('sp.js', 'SP.ClientContext', () => { /**/ }); // won't always load
  await waitForCondition(() => typeof _spPageContextInfo !== 'undefined', timeout, tries);
  return _spPageContextInfo;
};
